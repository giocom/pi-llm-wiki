...
| 버전 | 기능 |
|---|---|
| **v0.9** (현재) | compile/merge/query의 `default_lang` config (예: 한국어 응답) |
| v0.8 | 다중 source compile (`/wiki:merge`), dedup + `--force`, 태그 필터, 자동 wiki 컨텍스트 주입 |
...
선택적 config 키 (v0.9):
- `default_lang` — `/wiki:compile`, `/wiki:merge`, `/wiki:query`가 LLM에게 해당 언어로 답하라고 지시하는 언어 코드 (`ko`, `ja`, `en`, ...). 예: `"default_lang": "ko"`.
...
이 확장이 설치된 Pi 세션 안에서 동작하는 LLM 에이전트(Claude Code, Codex,
OpenCode 등)는 워크플로우·도구·핵심 원칙의 빠른 참조를
[AGENTS-ko.md](./AGENTS-ko.md)에서 읽을 수 있습니다. 영문판은
[AGENTS.md](./AGENTS.md). pi-llm-wiki의 영감이 된 llm-wiki의 더 큰
portable 프로토콜은
<https://github.com/nvk/llm-wiki/blob/master/AGENTS.md>에 있습니다.
...
### 자동 wiki 컨텍스트 주입 (v0.8 + v0.12)

매 agent 턴마다 `before_agent_start` 훅이 두 가지 선택적 주입을 수행합니다:

1. **AGENTS.md 에이전트 참조 (v0.11, v0.12에서 게이팅)** — 사용자
   프롬프트에 wiki 트리거 단어(`/wiki`, `wiki`, `워키`, `워크`,
   `정리해`, `요약해`, `컴파일`, `만들어`, `ingest`, `compile`,
   `query`, `merge`, `lint` 등)가 포함된 경우에만 주입.
   무관한 턴에 ~3-4k 토큰을 쓰지 않기 위함. `default_lang`이
   `ko`면 한국어판(`AGENTS-ko.md`) 우선.
2. **Wiki 컨텍스트 (v0.8)** — 항상 시도 (저비용 키워드 추출).
   프롬프트에서 최대 5개 키워드 추출 후 `wiki/`와 `raw/articles/`
   둘 다 grep. 매치가 있으면 `## Wiki context (auto-injected)`
   블록을 `path:line` 인용과 함께 추가. 매치 없으면 오버헤드 0.

이 덕분에 LLM이 매번 `/wiki:query`를 명시적으로 부르지 않아도
사용자의 wiki를 인용하며 답하고, wiki 관련 턴에서는 사용자가
붙여넣지 않아도 LLM이 도구 표 / 허브 구조 / 핵심 원칙을 알고
시작합니다. 컨텍스트 사용량 통제됨 — AGENTS.md는 트리거 시에만
~9KB, wiki 컨텍스트는 최악 ~500 토큰.
